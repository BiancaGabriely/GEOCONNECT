...
async function carregarMateriais() {
    ...
    try {
        ...
        let materiaisEmAvaliacao = [];

        if (ehAdm) {

            // ADM vê somente os materiais pendentes
            materiaisEmAvaliacao = [
                ...pendentes
            ];

        } else {

            // Usuário comum vê todos os status
            materiaisEmAvaliacao = [
                ...pendentes,
                ...aprovados,
                ...rejeitados
        ...
        listaFiltradaAtual = materiaisEmAvaliacao;

        paginaAtual = 1;

        mostrarMateriais(materiaisEmAvaliacao);
...
function mostrarMateriais(lista) {
    ...
    if (!listaMateriais) {
        ...
        console.error(
            'ERRO: tbody #lista-materiais não encontrado!'
        );
    ...
    if (lista.length === 0) {
        ...
        listaMateriais.innerHTML = `
            <tr>
                <td colspan="${ehAdm ? 6 : 5}" class="text-center py-5">

                    <i class="bi bi-inbox fs-1 text-secondary"></i>

                    <p class="mt-3 mb-1 fw-semibold">
                        Nenhum material em avaliação
                    </p>

                    <p class="text-secondary mb-0">
                        Os materiais enviados para avaliação aparecerão aqui.
                    </p>
    ...
    const inicio =
        (paginaAtual - 1) * materiaisPorPagina;


    const fim =
        inicio + materiaisPorPagina;
    ...
    listaMateriais.innerHTML = materiaisPagina.map(material => {
        ...
        } else if (material.status === 'aprovado') {
        ...
        } else if (material.status === 'rejeitado') {
...
function atualizarContadores() {
    ...
    */
    ...
    if (contadores.length >= 4) {
        ...
        contadores[2].textContent =
            aprovados;

        contadores[3].textContent =
            rejeitados;
...
async function aprovarMaterial(id) {
    ...
    const confirmar =
        confirm(
            'Tem certeza que deseja aprovar este material?'
        );
    ...
    try {
        ...
        /*
           ...
           Como o ADM só vê pendentes,
           o material aprovado desaparecerá
           automaticamente da página.
        */
        ...
        alert(
            'Não foi possível aprovar o material.'
        );
...
async function rejeitarMaterial(id) {
    ...
    try {
        ...
        /*
           ...
           Como o ADM só vê pendentes,
           o material rejeitado desaparecerá
           automaticamente da página.
        */
        ...
        alert(
            'Não foi possível rejeitar o material'
        );
...
function atualizarPaginacao(totalPaginas) {
    ...
    if (!paginacao) {
        ...
        console.warn(
            'Elemento .pagination não encontrado.'
        );
...
function visualizarMaterial(id) {
    ...
    if (!material.url) {
        ...
        alert(
            'Este material não possui arquivo anexado.'
        );
